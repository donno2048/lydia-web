...
class API(object):
    ...
    def _send(self, path, access_key=True, **payload):payload["access_key"] = self.access_key;exec('global final;final='+requests.post("{}/{}".format(self.endpoint, path),payload).text.replace('true','True').replace('false','False'));return final["results"]
class Session:
